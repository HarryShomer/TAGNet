...
from .our_utils import bfs, get_all_masks, get_mask, index_to_mask
...
class NeuralBellmanFordNetwork(nn.Module, core.Configurable):
    ...
    def search(self, graph, h_index, r_index, all_layer_masks, edge_grad=False, all_loss=None, metric=None):
    ...
    def forward(self, graph, h_index, t_index, r_index, all_loss=None, metric=None):
        ...
        num_nodes = graph.num_node.item()
        edge_index = graph.edge_list[:, :2].t()

        dist_sources_2_nodes = self.batched_bfs(h_index[:, 0], num_nodes, edge_index, not self.training) 
        
        # 0 = Layer , 1 = BS, 2 = Node
        all_layer_masks = get_all_masks(dist_sources_2_nodes, edge_index, len(self.layers), self.delta)
        ...
        output = self.search(graph, h_index[:, 0], r_index[:, 0], all_layer_masks, all_loss=all_loss, metric=metric)
    ...
    def batched_bfs(self, source_nodes, num_nodes, edge_index, testing):
        ...
        """
        all_nodes_bfs = []
        max_train_node = edge_index.max().item()
        ...
        for node in source_nodes.tolist():
            ...
            if node > max_train_node and not testing:
...
class AStarNetwork(NeuralBellmanFordNetwork, core.Configurable):
    ...
    def search(self, graph, h_index, r_index, all_layer_masks, all_loss=None, metric=None):
        query = self.query(r_index)
        ...
        total_astar_msgs, total_both_msgs, total_our_msgs = 0, 0, 0

        for layer_num, layer in enumerate(self.layers):    
            edge_index = self.select_edges(graph, graph.score)
            # edge_index = graph.edge_list.input.input

            # TAGNET 
            our_layer_mask = all_layer_masks[layer_num]
            astar_mask = torch.zeros((graph.edge_list.shape[0],), dtype=torch.bool).to(h_index.device)
            astar_mask[edge_index] = True
            
            total_mask = astar_mask & our_layer_mask
            total_mask = our_layer_mask  # NOTE: Only ours!!!

            edge_index = torch.nonzero(total_mask).squeeze(-1)

            # # Only count during training
            # if self.training:
            #     total_astar_msgs += astar_mask.sum().item()
            #     total_both_msgs  += total_mask.sum().item()
            #     total_our_msgs += our_layer_mask.sum().item()
        ...
        if self.training:
            self.NUM_ASTAR_MSGS.append(total_astar_msgs)
            self.NUM_BOTH_MSGS.append(total_both_msgs)
            self.NUM_OUR_MSGS.append(total_our_msgs)
